<div class="form-register">
    <form  v-on:submit.prevent="submitFormForRegistration()" class="">
        ...
        <div class="form-group mb-2">
            <label for="name" class="sr-only">Full_Name</label>
            <input id="name" :class="{ 'is-invalid':checkError('name')}" v-model.trim="input.name" type="text" v-on:keyup="validateMe($event,'name')" class="form-control" placeholder="Full Name">
            <div class="invalid-feedback c-err" v-if="checkError('name')">

                                    <span v-for="er in regError.name">
                                            @{{ er }}
                                    </span>
            </div>
        </div>
